...
import com.example.newbiechen.ireader.model.bean.BookInfoBean;
...
public class RemoteRepository {
    ...
    public Single<List<RecommenBeanCkass.MaleBean>> getRecommendStatistics(){
        return mBookApi.getRecommendStatistics()
                .map(beanCkass -> beanCkass.getMale());
    }

    public Single<List<RecommenBeanCkass.FemaleBean>> getRecommandFemal(){
        return mBookApi.getRecommendStatistics()
                .map(beanCkass -> beanCkass.getFemale());
    }

    public Single<BookListBean1> getBookList(){
        return mBookApi.getRecommandList()
                .map(bookListBean -> bookListBean);
    }

    public Single<List<BookCitySearchBean.SearchHotWordsBean>> getBookCitySearch(){
        return mBookApi.getBookSearch()
                .map(bookCitySearchBean -> bookCitySearchBean.getSearchHotWords());
    }
